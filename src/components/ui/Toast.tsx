...
export const NotificationBanner = memo<NotificationBannerProps>(({ message, type = TOAST_TYPES.INFO, isVisible = true, onClose, action, className = '' }) => {
    ...
    return (
        <motion.div initial={{ opacity: 0, clipPath: 'inset(0 0 100% 0)' }} animate={{ opacity: 1, clipPath: 'inset(0 0 0% 0)' }} exit={{ opacity: 0, clipPath: 'inset(0 0 100% 0)' }} transition={{ duration: 0.3 }} className={`${bannerColors[type]} border-l-4 p-4 ${className}`}>
