import React, { memo } from 'react';
import { motion } from 'framer-motion';
import { useScrollProgress, useScrollDirection } from '../../hooks/useScrollAnimation';

/**
 * 스크롤 진행률 인디케이터 컴포넌트
 * 페이지 상단에 스크롤 진행률을 표시하는 프로그레스 바
 */
const ScrollProgress = memo(({ className = '' }) => {
  const scrollProgress = useScrollProgress();
  const scrollDirection = useScrollDirection();

  return (
    <motion.div
      className={`fixed top-0 left-0 right-0 z-50 h-1 bg-gray-800/50 backdrop-blur-sm ${className}`}
      initial={{ opacity: 0 }}
      animate={{ 
        opacity: scrollProgress > 0.01 ? 1 : 0,
        y: scrollDirection === 'down' ? -4 : 0
      }}
      transition={{ duration: 0.3 }}
    >
      {/* 진행률 바 */}
      <motion.div
        className="h-full bg-gradient-to-r from-brand-primary-500 via-brand-solidarity-500 to-brand-harmony-500 shadow-lg"
        style={{
          width: `${scrollProgress * 100}%`,
        }}
        initial={{ width: 0 }}
        animate={{ width: `${scrollProgress * 100}%` }}
        transition={{ duration: 0.1, ease: 'linear' }}
      />
      
      {/* 글로우 효과 */}
      <motion.div
        className="absolute top-0 right-0 h-full w-20 bg-gradient-to-l from-brand-primary-400/30 to-transparent blur-sm"
        style={{
          right: `${100 - scrollProgress * 100}%`,
        }}
        animate={{
          opacity: scrollProgress > 0.1 ? 0.8 : 0,
        }}
        transition={{ duration: 0.3 }}
      />
    </motion.div>
  );
});

ScrollProgress.displayName = 'ScrollProgress';

export default ScrollProgress;