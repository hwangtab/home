...
interface SearchResultProps {
    ...
    onClick: (result: FuseResult<SearchItem>) => void;
    type: string;
}
...
const SearchResult: React.FC<SearchResultProps> = ({ result, onClick, type }) => {
    const getIcon = (itemType: string) => {
        switch (itemType) {
            ...
            case 'performance': return <Mic size={16} />;
            default: return <Hash size={16} />;
        }
    };

    const highlightMatches = (text: string, indices: readonly [number, number][]) => {
        ...
        >
            ...
            <div className="flex-1 min-w-0">
                <h4 className="text-gray-200 font-wanted-sans font-medium mb-1">
                    {result.matches?.find((m: FuseResultMatch) => m.key === 'title')?.indices ?
                        highlightMatches(result.item.title, result.matches.find((m: FuseResultMatch) => m.key === 'title')!.indices) :
                        result.item.title
                    }
                </h4>
                ...
                {result.item.description && (
                    <p className="text-gray-400 text-sm mt-1 line-clamp-2">
                        {result.matches?.find((m: FuseResultMatch) => m.key === 'description')?.indices ?
                            highlightMatches(
...
interface SearchBarProps {
    data: any[];
    onResultClick: (result: FuseResult<SearchItem>) => void;
    placeholder?: string;
}
...
const SearchBar: React.FC<SearchBarProps> = ({ data, onResultClick, placeholder = "작품, 연도 검색..." }) => {
    ...
    const searchData = React.useMemo<SearchItem[]>(() => {
        ...
        if (d.events?.concerts) {
            d.events.concerts.forEach((item: any) => {
                items.push({ ...item, type: 'event' });
            });
        }

        if (d.works) {
            Object.values(d.works).flat().forEach((work: any) => {
                if (typeof work === 'object') items.push({ ...work, type: work.archiveCategory || 'work' });
            });
        }
    ...
    const fuse = React.useMemo(() => {
        ...
        return new Fuse<SearchItem>(searchData, options);
    }, [searchData]);
    ...
    const handleResultClick = (result: FuseResult<SearchItem>) => {
        setQuery('');
        setIsOpen(false);
        setSelectedIndex(-1);
        inputRef.current?.blur();
