...
const Lightbox = ({ images = [], currentIndex = 0, isOpen = false, onClose, onImageChange }) => {
        ...
        >
          ...
          <div className="absolute top-0 left-0 right-0 z-60 p-4 bg-gradient-to-b from-black to-transparent">
            <div className="flex items-center justify-between text-white">
              <div className="flex items-center space-x-4">
                <span className="font-wanted-sans">
                  {currentIndex + 1} / {images.length}
                </span>
                {currentImage.title && (
                  <h3 className="font-santokki text-lg">{currentImage.title}</h3>
                )}
              </div>
              ...
              <div className="flex items-center space-x-2">
                ...
                <span className="px-2 py-1 bg-black bg-opacity-50 rounded text-sm">
                  {Math.round(zoom * 100)}%
                </span>
          ...
          {currentIndex > 0 && (
            <button
              ...
              className="absolute left-4 top-1/2 transform -translate-y-1/2 z-60 p-3 rounded-full bg-black bg-opacity-50 hover:bg-opacity-70 text-white transition-all"
            >
              <ChevronLeft size={24} />
            </button>
          )}
          ...
          {currentIndex < images.length - 1 && (
            <button
              ...
              className="absolute right-4 top-1/2 transform -translate-y-1/2 z-60 p-3 rounded-full bg-black bg-opacity-50 hover:bg-opacity-70 text-white transition-all"
            >
          ...
          {currentImage.description && (
            <div className="absolute bottom-20 left-4 right-4 z-60">
              <div className="bg-black bg-opacity-70 text-white p-4 rounded-lg max-w-md mx-auto">
                <p className="font-wanted-sans text-sm leading-relaxed">
                  {currentImage.description}
                </p>
              </div>
            </div>
