...
interface EasingConfig {
    EASE_OUT: EasingArray;
    EASE_IN_OUT: EasingArray;
    SPRING: SpringConfig;
    SMOOTH: EasingArray;
}
...
interface CommonAnimationsConfig {
    fadeIn: AnimationState;
    fadeInUp: AnimationState;
    fadeInDown: AnimationState;
...
interface CardAnimationConfig {
    whileHover?: HoverTapState | { scale: number };
...
// 표준화된 이징 커브
export const EASING: EasingConfig = {
...
// 공통 애니메이션 설정 - 성능 최적화
export const COMMON_ANIMATIONS: CommonAnimationsConfig = {
    // 페이드 인/아웃
    fadeIn: {
        initial: { opacity: 0 },
        animate: { opacity: 1 },
...
// 카드 애니메이션 프리셋 - 성능 최적화
export const CARD_ANIMATIONS: Record<string, CardAnimationConfig> = {
    default: {
...
// 페이지 전환 애니메이션 - 최적화
export const PAGE_ANIMATIONS: Record<string, AnimationState> = {
    fadeTransition: {
        initial: { opacity: 0 },
        animate: { opacity: 1 },
        exit: { opacity: 0 },
...
export const createStaggeredAnimation = (delayPerItem = 0.06): StaggerConfig => ({
    container: {
        animate: {
            transition: {
...
export const createDelayedAnimation = (delay = 0): AnimationState => ({
    initial: { opacity: 0, y: 15 },
    animate: { opacity: 1, y: 0 },
