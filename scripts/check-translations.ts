 ...
 * 빌드 전에 siteData.json의 한국어 원문에 대한 EN 번역 누락 체크.
 * KO_TO_EN map에 없는 문자열을 찾아 경고합니다.
 */
...
// ── KO_TO_EN map (must match src/utils/translateSiteData.ts) ──
...
function extractStrings(value: unknown, path = ''): string[] {
  const results: string[] = [];
  if (typeof value === 'string') {
    results.push(path || value);
  } else if (Array.isArray(value)) {
    value.forEach((item, i) => results.push(...extractStrings(item, `${path}[${i}]`)));
  } else if (value && typeof value === 'object') {
    for (const key of Object.keys(value as object)) {
      results.push(...extractStrings((value as Record<string, unknown>)[key], `${path}.${key}`));
    }
  }
  return results;
}
...
function main(): void {
  ...
  // Only check works (where Korean text lives that needs inline translation)
  const allStrings = new Set<string>();
  const workKeys = ['music', 'visual', 'writing', 'performance', 'struggle'];
  for (const wk of workKeys) {
    const items = (data.works as Record<string, unknown[]>)[wk];
    if (!items) continue;
    for (const item of items) {
      const strList = extractStrings(item);
      // Skip JSON key paths (containing dots/brackets), keep only raw string values
  ...
  if (untranslated.length > 0) {
    console.error('\n⚠️ 번역 누락 감지된 한국어 원문 (EN map에 없음):');
    for (const s of untranslated.sort()) {
      console.error(`  - "${s.substring(0, 80)}${s.length > 80 ? '...' : ''}"`);
    }
    console.error(`\n총 ${untranslated.length}개 항목 번역이 필요합니다.`);
    console.error('src/utils/translateSiteData.ts의 KO_TO_EN에 추가하세요.\n');
    process.exit(1);
  }

  console.log(`✅ siteData.json 한국어 원문 ${allStrings.size}개 모두 번역 매핑 완료.`);
  process.exit(0);
}
